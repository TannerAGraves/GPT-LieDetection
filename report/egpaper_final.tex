...
This scenario involved the aggregation of three distinct train and test sets from \texttt{'Scenario 1'}, 
...
To keep costs low, we decided to perform 3-fold cross validation only on
...
As expected, the \texttt{300-Model} performed worse than the \texttt{Full-Model}. The former exhibited a 
remarkable improvement in performance, achieving an overall accuracy of 65.5\% (!!!!!!!!!!!!!), whereas the latter achieved an overall accuracy of \dots
This observation suggests that when fine-tuning, performances positively scale with the size
...
of accuracy when it came to the non-cross-validated \texttt{Full-Model}. 
